...
export async function GET(request: NextRequest) {
  try {
    ...
    // Category filter - handle both categoryId and categoryName
    let categoryFilterId = categoryId;
    if (categoryName && categoryName !== "all" && !categoryId) {
      // If categoryName is provided but not categoryId, find the category by name
      // The categoryName comes as slug (e.g., "living-room"), convert to title case for DB lookup
    ...
    if (categoryFilterId && categoryFilterId !== "all") {
      where.categoryId = categoryFilterId;
    }
